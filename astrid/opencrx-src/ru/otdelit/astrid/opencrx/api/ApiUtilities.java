...
public final class ApiUtilities {
    ...
    public static Metadata createNoteMetadata(String note) {
        Metadata metadata = new Metadata();
        metadata.setValue(Metadata.KEY, NoteMetadata.METADATA_KEY);
        metadata.setValue(NoteMetadata.EXT_ID, "");
        metadata.setValue(NoteMetadata.EXT_PROVIDER, OpencrxDataService.NOTE_PROVIDER);
        metadata.setValue(NoteMetadata.BODY, note);

        Time cur = new Time();
        cur.setToNow();
        metadata.setValue(Metadata.CREATION_DATE, cur.toMillis(false));

        metadata.setValue(NoteMetadata.TITLE, DateUtilities.getDateStringWithWeekday(ContextManager.getContext(),
                new Date(cur.toMillis(false))));
        return metadata;
    }
}
