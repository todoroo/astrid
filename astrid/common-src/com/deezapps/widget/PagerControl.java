...
public class PagerControl extends View {
    ...
    private final Paint barPaint, highlightPaint;
    private final int fadeDelay, fadeDuration;
    ...
    public void setNumPages(int numPages) {
        if (numPages <= 0) {
            throw new IllegalArgumentException("numPages must be positive"); //$NON-NLS-1$
        }
    ...
    public void setCurrentPage(int currentPage) {
        if (currentPage < 0 || currentPage >= numPages) {
            throw new IllegalArgumentException("currentPage parameter out of bounds"); //$NON-NLS-1$
        }
