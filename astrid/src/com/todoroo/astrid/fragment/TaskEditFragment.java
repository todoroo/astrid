...
public final class TaskEditFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // this call is needed if tabHost isn't retrieved by TabActivity.getTabHost()
        tabHost.setup();
    ...
    private void setUpUIComponents() {
        ...
        notesEditText = (EditText) getView().findViewById(R.id.notes);
        ...
        if(addOnService.hasPowerPack()) {
            voiceAddNoteButton = (ImageButton) getView().findViewById(R.id.voiceAddNoteButton);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    public void run() {
                        // internal add-ins
                        controls.add(new TagsControlSet(getActivity(), R.id.tags_container));

                        LinearLayout extrasAddons = (LinearLayout) getView().findViewById(R.id.tab_extra_addons);
                        controls.add(new RepeatControlSet(getActivity(), extrasAddons));
                        controls.add(new GCalControlSet(getActivity(), extrasAddons));

                        LinearLayout addonsAddons = (LinearLayout) getView().findViewById(R.id.tab_addons_addons);

                        try {
                            if(ProducteevUtilities.INSTANCE.isLoggedIn()) {
                                controls.add(new ProducteevControlSet(getActivity(), addonsAddons));
                                notesEditText.setHint(R.string.producteev_TEA_notes);
                                ((TextView)getView().findViewById(R.id.notes_label)).setHint(R.string.producteev_TEA_notes);
                            }
                        } catch (Exception e) {
                        ...
                        try {
                            if(OpencrxCoreUtils.INSTANCE.isLoggedIn()) {
                                controls.add(new OpencrxControlSet(getActivity(), addonsAddons));
                                notesEditText.setHint(R.string.opencrx_TEA_notes);
                                ((TextView)getView().findViewById(R.id.notes_label)).setHint(R.string.opencrx_TEA_notes);
                            }
                        } catch (Exception e) {
                            Log.e("astrid-error", "loading-control-set", e); //$NON-NLS-1$ //$NON-NLS-2$
                        }

                        controls.add(new TimerControlSet(getActivity(), addonsAddons));
                        controls.add(new AlarmControlSet(getActivity(), addonsAddons));

                        if(!addOnService.hasPowerPack()) {
                            // show add-on help if necessary
                            View addonsEmpty = getView().findViewById(R.id.addons_empty);
                            addonsEmpty.setVisibility(View.VISIBLE);
                            addonsAddons.removeView(addonsEmpty);
                            addonsAddons.addView(addonsEmpty);
                            ((Button)getView().findViewById(R.id.addons_button)).setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View view) {
                                    Intent addOnActivity = new Intent(getActivity(), AddOnActivity.class);
    ...
    private void setUpListeners() {
        ...
        try {
            ImageButton saveButtonGeneral = (ImageButton) getView().findViewById(R.id.save_basic);
            saveButtonGeneral.setOnClickListener(mSaveListener);
            ImageButton saveButtonDates = (ImageButton) getView().findViewById(R.id.save_extra);
            saveButtonDates.setOnClickListener(mSaveListener);
            ImageButton saveButtonNotify = (ImageButton) getView().findViewById(R.id.save_addons);
            saveButtonNotify.setOnClickListener(mSaveListener);

            ImageButton discardButtonGeneral = (ImageButton) getView().findViewById(R.id.discard_basic);
            discardButtonGeneral.setOnClickListener(mDiscardListener);
            ImageButton discardButtonDates = (ImageButton) getView().findViewById(R.id.discard_extra);
            discardButtonDates.setOnClickListener(mDiscardListener);
            ImageButton discardButtonNotify = (ImageButton) getView().findViewById(R.id.discard_addons);
     ...
     * ====================================================================== */
    protected void loadItem(Intent intent, boolean refresh) {
    ...
    protected void loadItem(Intent intent) {
        loadItem(intent, false);
    }

    public void populateFields(Intent intent, boolean refresh) {
        Resources r = getResources();
        Intent intentToUse = (intent != null ? intent : getActivity().getIntent());
        loadItem(intentToUse, refresh);

        if(isNewTask)
            getActivity().setTitle(R.string.TEA_view_titleNew);
        else
            getActivity().setTitle(r.getString(R.string.TEA_view_title, model.getValue(Task.TITLE)));
    ...
    protected class ControlReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            try {
                ...
                LinearLayout dest = (LinearLayout)getView().findViewById(R.id.tab_addons_addons);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch(item.getItemId()) {
            case android.R.id.home:
                ...
                Intent intent = new Intent(this.getActivity(), FilterListActivity.class);
    ...
    public class ImportanceControlSet implements TaskEditControlSet {
        ...
        public ImportanceControlSet(int containerId) {
            LinearLayout layout = (LinearLayout)getView().findViewById(containerId);
    ...
    private class UrgencyControlSet implements TaskEditControlSet,
        ...
        public UrgencyControlSet(int urgency) {
            this.spinner = (Spinner)getView().findViewById(urgency);
        ...
        private void createUrgencyList(long dueDate) {
            ...
            if(selection == -1) {
                ...
                if(Task.hasDueTime(dueDate)) {
                    ...
                    updated[0] = new UrgencyValue(DateUtilities.getDateStringWithTime(getActivity(), dueDateAsDate),
                    ...
                    updated[0] = new UrgencyValue(DateUtilities.getDateString(getActivity(), new Date(dueDate)),
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            ...
            if(item.dueDate == SPECIFIC_DATE) {
                ...
                final CalendarDialog calendarDialog = new CalendarDialog(getActivity(), customDate);
                calendarDialog.show();
        ...
        private void setDate(CalendarDialog calendarDialog) {
            ...
            DeadlineTimePickerDialog timePicker = new DeadlineTimePickerDialog(getActivity(), this,
    ...
    private class HideUntilControlSet implements TaskEditControlSet,
        ...
        public HideUntilControlSet(int hideUntil) {
            this.spinner = (Spinner) getView().findViewById(hideUntil);
            this.spinner.setOnItemSelectedListener(this);
        }
        ...
        private HideUntilValue[] createHideUntilList(long specificDate) {
            ...
            if(specificDate > 0) {
                ...
                if(hideUntilAsDate.getHours() == 0 && hideUntilAsDate.getMinutes() == 0 && hideUntilAsDate.getSeconds() == 0) {
                    updated[0] = new HideUntilValue(DateUtilities.getDateString(getActivity(), new Date(specificDate)),
                            Task.HIDE_UNTIL_SPECIFIC_DAY, specificDate);
                    existingDate = specificDate;
                    existingDateHour = SPECIFIC_DATE;
                } else {
                    updated[0] = new HideUntilValue(DateUtilities.getDateStringWithTime(getActivity(), new Date(specificDate)),
    ...
    public class ReminderControlSet implements TaskEditControlSet {
        ...
        public ReminderControlSet(int duringId, int afterId, int modeId) {
            during = (CheckBox)getView().findViewById(duringId);
            after = (CheckBox)getView().findViewById(afterId);
            mode = (Spinner)getView().findViewById(modeId);
    ...
    public class RandomReminderControlSet implements TaskEditControlSet {
        ...
        public RandomReminderControlSet(int settingCheckboxId, int periodButtonId) {
            settingCheckbox = (CheckBox)getView().findViewById(settingCheckboxId);
            periodSpinner = (Spinner)getView().findViewById(periodButtonId);
